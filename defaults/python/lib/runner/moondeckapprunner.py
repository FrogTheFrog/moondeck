...
class MoonDeckAppRunner:
    @staticmethod
    async def check_connectivity(client: BuddyClient, overlay_stack: OverlayStack, mac: str, host_id: str, hostname: str, host_port: int, wol_port: int, send_wol_once: bool, custom_wol_exec: Optional[str], wol_timeout: int, server_timeout: int):
        logger.info("Checking connection to Buddy and GameStream server")

        # Lazy import to improve CLI performance
        from .wolsplashscreen import WolSplashScreen

        async with WolSplashScreen(overlay_stack, False, False, client.address, mac, wol_timeout, hostname, wol_port, send_wol_once, custom_wol_exec) as splash:
            while True:
                try:
